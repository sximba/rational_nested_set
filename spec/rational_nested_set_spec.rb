...
describe "RationalNestedSet" do
  before(:all) do
    self.class.fixtures :categories, :departments, :notes, :things, :brokens, :users, :default_scoped_models
  end

  describe "defaults" do
    it "should have numv_default" do
      expect(Default.acts_as_nested_set_options[:numv_column]).to eq('numv')
    end

    it "should have denv_default" do
      expect(Default.acts_as_nested_set_options[:denv_column]).to eq('denv')
    end

    it "should have snumv_default" do
      expect(Default.acts_as_nested_set_options[:snumv_column]).to eq('snumv')
    end

    it "should have sdenv_default" do
      expect(Default.acts_as_nested_set_options[:sdenv_column]).to eq('sdenv')
    end

    it "should have total_order_default" do
      expect(Default.acts_as_nested_set_options[:total_order_column]).to eq('total_order')
    end

    it "should have is_leaf_default" do
      expect(Default.acts_as_nested_set_options[:is_leaf_column]).to eq('is_leaf')
    end
  ...
  it "creation when existing record has nil numv,denv,snumv,sdenv columns" do
    expect {
      Broken.create!
    }.not_to raise_exception
  end
  ...
  describe "quoted column names" do
    it "quoted_numv_column_name" do
      quoted = Default.connection.quote_column_name('numv')
      expect(Default.quoted_numv_column_name).to eq(quoted)
      expect(Default.new.quoted_numv_column_name).to eq(quoted)
    end

    it "quoted_denv_column_name" do
      quoted = Default.connection.quote_column_name('denv')
      expect(Default.quoted_denv_column_name).to eq(quoted)
      expect(Default.new.quoted_denv_column_name).to eq(quoted)
    end

    it "quoted_snumv_column_name" do
      quoted = Default.connection.quote_column_name('snumv')
      expect(Default.quoted_snumv_column_name).to eq(quoted)
      expect(Default.new.quoted_snumv_column_name).to eq(quoted)
    end

    it "quoted_sdenv_column_name" do
      quoted = Default.connection.quote_column_name('sdenv')
      expect(Default.quoted_sdenv_column_name).to eq(quoted)
      expect(Default.new.quoted_sdenv_column_name).to eq(quoted)
    end
  ...
  describe "protected columns" do
    it "numv_column_protected_from_assignment" do
      expect {
        Category.new.numv = 1
      }.to raise_exception(ActiveRecord::ActiveRecordError)
    end

    it "denv_column_protected_from_assignment" do
      expect {
        Category.new.denv = 1
      }.to raise_exception(ActiveRecord::ActiveRecordError)
    end

    it "snumv_column_protected_from_assignment" do
      expect {
        Category.new.numv = 1
      }.to raise_exception(ActiveRecord::ActiveRecordError)
    end

    it "sdenv_column_protected_from_assignment" do
      expect {
        Category.new.denv = 1
      }.to raise_exception(ActiveRecord::ActiveRecordError)
    end
  ...
  describe "hierarchical structure" do
    it "roots_class_method" do
      found_by_us = Category.where(:denv => 1).to_a
